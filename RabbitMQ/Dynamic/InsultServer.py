...
class InsultPublisher:
    ...
    def publish_insults(self):
        # Publish a message
        while True:
            text = self.filter_insult(random.choice(self.insultList))
            self.channel.basic_publish(exchange='', routing_key='insultQueue', body=text)
...
def start_server(queue_name, stop_event):
    insult_serv = InsultPublisher(queue_name)  # Create the InsultServer
    while not stop_event.is_set():
        insult_serv.publish_insults()  # Add insult to the queue
        time.sleep(0.1)  # Simulate some delay between adding insults
    insult_serv.close_connection()