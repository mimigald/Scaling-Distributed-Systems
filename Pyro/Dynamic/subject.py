...
class Observable:
    ...
    def filter_message(self, message):
        ...
        for word in words:
            clean_word = word.lower().strip(".,!?")  # Convertir a minúsculas y eliminar puntuaciones
            if clean_word in (insult.lower() for insult in self.blacklist):  # Comparación en minúsculas
                filtered_words.append("CENSORED")
            else:
                filtered_words.append(word)

        censored_message = " ".join(filtered_words)  # Unir las palabras nuevamente
        #self.notify_observers(censored_message)
    ...
    def insult_me(self):
        return self.filter_message(random.choice(self.insultList))
...
def start_server(name_server):
    daemon = Pyro4.Daemon()
    ns = Pyro4.locateNS()
    uri = daemon.register(Observable)
    ns.register(name_server, uri)
    print(f"Observable {name_server} server is running. URI: {uri}")
    daemon.requestLoop()
